...
| [终端重新挂载焦点](docs/163-terminal-attachment-focus.md) | 按实际附着重试、取消过期请求及旧宿主隔离；桌面键盘复验待解锁 |
...
下一步继续推进 Codex Core 适配和隔离验证；兼容 API 文本会话已接入，真实服务验证等待用户填写设置，Agent 工具循环尚未接入。路线图中的未来能力不代表当前二进制已经支持。
