...
struct ArchivedTaskPresentation {
  ...
  init(library: WorkspaceLibrary, runs: [AgentRun] = [], automationTaskIDs: Set<String> = []) {
    knownProjects = Dictionary(library.projects.filter { !$0.isEmpty }.map { ($0, library.projectTitle($0)) },
    ...
    entries = library.tasks.filter { $0.archived && !$0.isPopoutDraft }.map { original in
      ...
      return ArchivedTaskEntry(task: task, projectTitle: library.projectTitle(task.project),
  ...
  var projects: [(path: String, title: String)] {
    ...
    for entry in entries where !entry.task.project.isEmpty { values[entry.task.project] = entry.projectTitle }
  ...
  func groups(query: String, project: ArchivedProjectFilter, kind: ArchivedTaskKind,
    ...
    let matches = entries.filter { entry in
      ...
      case .project(let path): include = entry.task.project == path && !entry.isAutomation
      ...
      let basename = entry.task.project.split(whereSeparator: { $0 == "/" || $0 == "\\" }).last.map(String.init) ?? ""
    ...
    var groups = Dictionary(grouping: matches, by: { $0.task.project }).map { path, entries in
