...
final class DesktopFeaturesTests: XCTestCase {
  ...
  @MainActor func testCodexTurnDiffClearRemovesPersistedCard() throws {
    ...
    let savedDiff = try XCTUnwrap(store.library.chatRuns.first?.codexTurnDiff)
    XCTAssertEqual(try CodexTurnDiffStorage.load(savedDiff, root: root),
    ...
    XCTAssertThrowsError(try CodexTurnDiffStorage.load(savedDiff, root: root))
  }

  @MainActor func testLargeCodexTurnDiffKeepsExactSnapshotAndUpdatesBeyondPreview() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    defer { try? FileManager.default.removeItem(at: root) }
    let store = WorkspaceStore(dataRoot: root)
    store.libraryLoaded = true
    ...
    store.library.chatRuns = [run]
    store.runs = [run]
    let prefix = "diff --git a/large.swift b/large.swift\n" + String(repeating: "+abcdefghij\n", count: 27_000)
    let first = prefix + "+first-tail\n"
    store.recordCodexTurnDiff(runID: run.id, event: .object([
      "type": .string("turn_diff"), "unified_diff": .string(first),
    ]))
    let initial = try XCTUnwrap(store.library.chatRuns.first?.codexTurnDiff)
    XCTAssertTrue(initial.truncated)
    XCTAssertEqual(initial.unifiedDiff.count, 262_144)
    XCTAssertEqual(initial.storedByteCount, first.utf8.count)
    XCTAssertEqual(try CodexTurnDiffStorage.load(initial, root: root), first)
    let second = prefix + "+second-tail\n"
    store.recordCodexTurnDiff(runID: run.id, event: .object([
      "type": .string("turn_diff"), "unified_diff": .string(second),
    ]))
    let updated = try XCTUnwrap(store.library.chatRuns.first?.codexTurnDiff)
    XCTAssertEqual(updated.id, initial.id)
    XCTAssertEqual(updated.unifiedDiff, initial.unifiedDiff)
    XCTAssertNotEqual(updated.contentSHA256, initial.contentSHA256)
    XCTAssertEqual(try CodexTurnDiffStorage.load(updated, root: root), second)
    let restored = try WorkspaceLibrary.load(from: root.appendingPathComponent("workspace.json"))
    XCTAssertEqual(try CodexTurnDiffStorage.load(XCTUnwrap(restored.chatRuns[0].codexTurnDiff),
      root: root), second)
  }

  @MainActor func testCodexTurnDiffSnapshotSurvivesForkUntilLastTaskDeleted() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    defer { try? FileManager.default.removeItem(at: root) }
    let store = WorkspaceStore(dataRoot: root)
    store.libraryLoaded = true
    let run = AgentRun(id: UUID().uuidString, kind: "chat", project: "/project",
      ...
      request: .object(["api_protocol": .string("codexResponses")]),
      result: .object(["response": .string("Done")]))
    store.library.attach(run, to: nil, note: "Original")
    store.library.chatRuns = [run]
    store.runs = [run]
    let source = "diff --git a/a.swift b/a.swift\n+new\n"
    store.recordCodexTurnDiff(runID: run.id, event: .object([
      "type": .string("turn_diff"), "unified_diff": .string(source),
    ]))
    let original = try XCTUnwrap(store.library.chatRuns.first)
    let fork = try store.library.forkConversation(taskID: run.id, availableRuns: [original])
    let forkDiff = try XCTUnwrap(store.library.forkRuns.first?.codexTurnDiff)
    store.library.tasks[store.library.tasks.firstIndex { $0.id == run.id }!].archived = true
    XCTAssertTrue(store.deleteArchivedTasks([run.id]))
    XCTAssertEqual(try CodexTurnDiffStorage.load(forkDiff, root: root), source)
    store.library.tasks[store.library.tasks.firstIndex { $0.id == fork.id }!].archived = true
    XCTAssertTrue(store.deleteArchivedTasks([fork.id]))
    XCTAssertThrowsError(try CodexTurnDiffStorage.load(forkDiff, root: root))
  }
...
final class ModelTransportTests: XCTestCase {
  ...
  @MainActor func testNewWorktreeTaskCopiesTrackedUntrackedAndIncludedIgnoredFiles() async throws {
    ...
    try Data(".env\nskip.env\n".utf8).write(to: source.appendingPathComponent(".gitignore"))
    try Data(".env\n".utf8).write(to: source.appendingPathComponent(".worktreeinclude"))
    _ = try await GitReviewService.checked(["add", "README.md", ".gitignore", ".worktreeinclude"], at: source)
    _ = try await GitReviewService.checked(["commit", "-qm", "Initial"], at: source)
    ...
    try Data("excluded\n".utf8).write(to: source.appendingPathComponent("skip.env"))
    ...
    XCTAssertEqual(managed.sourceCopiedFiles?.map(\.path).sorted(), [".env", "untracked.txt"])
    XCTAssertEqual(store.library.managedWorktrees.first?.sourceChangesApplied, true)
    XCTAssertFalse(FileManager.default.fileExists(atPath: root.appendingPathComponent("Data")
      .appendingPathComponent("ManagedSourceSnapshots")
      .appendingPathComponent(task.id).path))
    let protectedRef = try await LocalWorkspaceService.git(
      ["show-ref", "--verify", "refs/shipios/managed-worktrees/\(task.id)"], at: source)
    XCTAssertNotEqual(protectedRef.status, 0)
    let checkout = URL(fileURLWithPath: managed.path)
    XCTAssertEqual(try String(contentsOf: checkout.appendingPathComponent("README.md"),
      encoding: .utf8), "local change\n")
    XCTAssertEqual(try String(contentsOf: checkout.appendingPathComponent("untracked.txt"),
      encoding: .utf8), "keep\n")
    XCTAssertEqual(try String(contentsOf: checkout.appendingPathComponent(".env"),
      encoding: .utf8), "included\n")
    XCTAssertFalse(FileManager.default.fileExists(atPath: checkout.appendingPathComponent("skip.env").path))
    ...
    XCTAssertEqual(try String(contentsOf: source.appendingPathComponent(".env"),
      encoding: .utf8), "included\n")
    await store.shutdown()
  }
  ...
  @MainActor func testCodexPatchAppearsInTimelineAndWritesProjectFile() async throws {
    ...
    XCTAssertEqual(try CodexTurnDiffStorage.load(diff, root: root.appendingPathComponent("Data")),
      diff.unifiedDiff)
  ...
  @MainActor func testCodexResponsesMCPFormElicitationReturnsTypedContent() async throws {
    ...
    let form = try XCTUnwrap(request, "MCP form did not reach the task timeline")
    XCTAssertEqual(form.fields.map(\.id), ["count", "reason"])
    XCTAssertFalse(form.validContent(.object(["reason": .string("ok"), "count": .string("2")])))
    XCTAssertFalse(form.validContent(.object(["reason": .string("no"), "count": .number(2)])))
    XCTAssertFalse(form.validContent(.object(["reason": .string("fixture reason")])))
    XCTAssertFalse(FileManager.default.fileExists(atPath: callLog.path))
    let content = JSONValue.object(["reason": .string("fixture reason"), "count": .number(2)])
    ...
    XCTAssertEqual(finished.codexElicitations.first?.status, .accepted)
    XCTAssertTrue(finished.responseItems?.contains(.elicitation(form.id)) == true)
    let answer = try String(contentsOf: callLog, encoding: .utf8)
    XCTAssertTrue(answer.contains("fixture reason"), answer)
    XCTAssertTrue(answer.contains("\"count\": 2"), answer)

    store.newTask()
    await store.startChat("codex-mcp-form-probe")
    let declinedRun = try XCTUnwrap(store.library.chatRuns.last)
    let declineDeadline = Date().addingTimeInterval(15)
    var declineForm: CodexElicitationRequest?
    while Date() < declineDeadline {
      declineForm = store.library.chatRuns.first(where: { $0.id == declinedRun.id })?.codexElicitations.last
      if declineForm != nil { break }
      try await Task.sleep(for: .milliseconds(50))
    }
    store.submitCodexElicitation(try XCTUnwrap(declineForm).id, accepted: false, content: nil)
    await store.modelTask(runID: declinedRun.id)?.value
    let declined = try XCTUnwrap(store.library.chatRuns.first { $0.id == declinedRun.id })
    XCTAssertEqual(declined.status, "succeeded", declined.result?["message"].text ?? "")
    XCTAssertEqual(declined.codexElicitations.first?.status, .declined)
