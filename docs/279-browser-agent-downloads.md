...
真实 WebKit 回归覆盖授权拒绝、同站文件下载、记录持久化、跨任务查询阻断、跨站链接和重定向阻断、慢速下载取消；本阶段浏览器套件 58 项通过。假模型 Codex Core 回合验证 `download` 的任务标签与短期句柄能往返 Agent 和宿主。已检查的同站链接受支持；普通 iframe 的授权与同站链接下载随后在[第 280 篇](280-browser-agent-frames.md)补齐。按钮生成、浏览器扩展下载和当前 Codex Mac 的逐页原生视觉及交互配对仍待完成。桌面工具本轮明确报告 Mac 锁定，因此 43 个完整配对验收面仍为 0。
