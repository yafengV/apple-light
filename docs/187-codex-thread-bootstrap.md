...
2026-09-26。在[固定版本嵌入 PoC](185-codex-core-embed-poc.md)的基础上，用公开 `codex-core-api` 实际构造 `ThreadManager`，创建并关闭一个线程。`./script/check_codex_core_embed.sh` 在离线 Cargo 模式下编译并运行成功；没有提交模型回合，也没有使用用户的 API 凭据。
...
产品 `shipios-agent` 尚未链接这一 PoC。流式回合事件、用户输入、工具注册和调用、审批、取消、恢复以及用户独立 API 服务的认证与模型协议仍未验证；UI 不能据此标为对齐。下一步应以本地受控模型服务驱动一个真实回合，先验证事件、错误与停止的稳定契约，再映射到 Swift 会话卡片和输入区。
